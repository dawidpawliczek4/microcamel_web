...
(* Import your session store and middleware *)
module Session_store = Session_store
module Session_middleware = Session_middleware

(* Example handler that uses sessions *)
let handle_home req _body _params =
  let session = Session_middleware.get_session req in
  let* visits =
    match Session_store.In_memory_store.get session "visits" with
    | Some v -> Lwt.return (int_of_string v)
    | None -> Lwt.return 0
  in
  let* () = Session_store.In_memory_store.set session "visits" (string_of_int (visits + 1)) in
